...
def test_to_fcc_qs_params_datetime_missing_value_ignored() -> None:
    assert not flt.to_fcc_qs_params({'date_changed': ''})

@pytest.mark.parametrize('incorrect_value', ['test', '123'])
def test_to_fcc_qs_params_datetime_incorrect_value_raises_error(incorrect_value: str) -> None:
    with pytest.raises(FilterError):
        flt.to_fcc_qs_params({'date_changed': incorrect_value})
